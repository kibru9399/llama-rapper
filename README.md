...
4. **Refinement through RLHF using PPO:** To temper the inherent propensity for undesirable language, the llama2 model undergoes further refinement. The integration of the reward model acts as a vigilant gatekeeper, instilling a sense of responsibility by discouraging the generation of toxic or inappropriate words. A base model is used to prevent the model from reward hacking and generate a similar but non-toxic response as the base model. the base model is the same model before RLHF
